...
float bytes_to_float(const std::array<std::byte, 2>& byte_array) {
  std::cout << " int8_t: " << std::bitset<8>(std::int8_t()) << " uint8_t: " << std::bitset<8>(std::uint8_t())
            << std::endl;
  std::int16_t scaled_value_int{};
  std::memcpy(&scaled_value_int, byte_array.data(), 2);
  std::cout << "scaled_value_int: " << scaled_value_int << " (bits): " << std::bitset<16>(scaled_value_int)
            << std::endl;
