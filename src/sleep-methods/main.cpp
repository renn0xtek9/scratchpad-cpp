...
void benchmark_all_methods_for_sleep_time_nominal() {
  ...
  std::vector<BenchmarkCondition> benchmark_conditions{
      ...
      BenchmarkCondition("CPU TIME ID + mm pause (stressed)", desired_duration,
                         with_clock_cpu_process_time_id_and_mm_pause, true),
  ...
  for (const auto& benchmark_condition : benchmark_conditions) {
    ...
    results.emplace(benchmark_condition.name_,
                    benchmark(benchmark_condition.desired_duration_, benchmark_condition.custom_sleep_function_,
                              benchmark_condition.stress_system_));
  }

  const auto executable_path = std::filesystem::path(get_path_of_current_executable());
  const std::filesystem::path csv_path =
      executable_path.parent_path().parent_path() / "share" / "sleep-methods" / "sleep-benchmark-file.csv";

  std::ofstream output_file(csv_path.string().c_str());
  output_file << "Sleep function;Desired;CPU "
                 "ticks;high_resolution_clock::now;system_clock;CLOCK_MONOTONIC;CLOCK_REALTIME;CLOCK_PROCESS_CPU_TIME_"
  ...
  for (const auto& result : results) {
    ...
    output_file << std::chrono::duration_cast<std::chrono::duration<double>>(
                       result.second.high_resolution_clock_duration)
                       .count()
                << ";";
    output_file
        << std::chrono::duration_cast<std::chrono::duration<double>>(result.second.system_clock_duration).count()
        << ";";
...
void benchmark_clock_real_time_distribution(const std::string csv_filename, const bool stress) {
  ...
  BenchmarkCondition benchmark_condition("REALTIME + mm pause (stressed)", desired_duration,
                                         with_clock_realtime_and_mm_pause, stress);
  std::vector<Duration> results{};
  for (int i = 0; i < result_size; i++) {
    results.push_back(benchmark(benchmark_condition.desired_duration_, benchmark_condition.custom_sleep_function_,
                                benchmark_condition.stress_system_));
  }

  const auto executable_path = std::filesystem::path(get_path_of_current_executable());
  const std::filesystem::path csv_path =
      executable_path.parent_path().parent_path() / "share" / "sleep-methods" / csv_filename.c_str();

  std::ofstream output_file(csv_path.string().c_str());
  output_file << "Iteration;Duration\n";
  for (int index = 0; index < results.size(); index++) {
    output_file << index << ";";
    output_file << std::abs(desired_duration - static_cast<double>(results.at(index).clock_realtime_duration) / 1.0e9) *
                       1.0e6
                << ";";
...
int main() {
  std::cout << "Note: CPU frequency (GHz): " << cpu_frequency / 1.0e9 << " -- Clock ticks per seconds "
            << clock_ticks_per_seconds << " -- CPU load " << get_cpu_load() << std::endl;
