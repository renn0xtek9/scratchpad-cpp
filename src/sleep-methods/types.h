...
struct TimePoint {
  TimePoint()
      : system_clock_time(std::chrono::system_clock::now()),
...
struct Duration {
  Duration(const TimePoint& start, const TimePoint& end)
      : clock_monotonic_duration(durationBetweenTimespecs(start.clock_monotonic_time, end.clock_monotonic_time)),
        ...
        clock_process_cputime_id_duration(
            durationBetweenTimespecs(start.clock_process_cputime_id_time, end.clock_process_cputime_id_time)),
        clock_thread_cputime_id_duration(
            durationBetweenTimespecs(start.clock_thread_cputime_id_time, end.clock_thread_cputime_id_time)),
        system_clock_duration(
            std::chrono::duration_cast<std::chrono::nanoseconds>(end.system_clock_time - start.system_clock_time)
                .count()),
...
struct BenchmarkCondition {
  BenchmarkCondition(const std::string name,
                     const double desired_duration,
                     const std::function<void(const double)>& custom_sleep_function,
                     const bool stress_system = false)
      : name_(name),
        desired_duration_(desired_duration),
        custom_sleep_function_(custom_sleep_function),
        stress_system_(stress_system) {
  }
